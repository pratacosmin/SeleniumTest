...
class TestDatasetSearch(unittest.TestCase):
    ...
    def test_Search_dataset(self):
        ...
        element = driver.find_element_by_id('icon_prefix')
        element.clear()
        element.send_keys('solubility')
        ...
        #Clicking View
        wait(driver, 10).until(EC.element_to_be_clickable((By.XPATH, ".//td/i[.='visibility']"))).click()
        
        time.sleep(5)
        
        wait(driver, 5).until(EC.element_to_be_clickable((By.XPATH, "//a[contains(text(),'Cancel')]"))).click()
